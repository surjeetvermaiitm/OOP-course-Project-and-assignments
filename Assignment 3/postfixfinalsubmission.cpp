...
charList charList :: infix_to_postfix(){
    ...
    postfix2[k]='\0';
    int m;
    for( k=0;postfix2[k]!='\0';k++){
        post.setvalue(k,postfix2[k]);
    }
    post.setvalue(k,'\0');
	...
	for (int i = 0; elements[i]!='\0'; i++) {
		...
		if (d >= '0' && d <= '9') {
            ...
            for (int j=0;j<size;j++) {
                result += (temp[j]-'0') * pow(10,size-1-j);
            }
            //cout << result << endl;
            s.push(result);
            temp="";
